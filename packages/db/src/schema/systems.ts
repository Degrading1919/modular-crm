...
}, (t) => [uniqueIndex("ticket_types_key_ux").on(t.tenantId, t.key)]);
...
}, (t) => [uniqueIndex("ticket_statuses_key_ux").on(t.tenantId, t.key)]);
...
}, (t) => [index("tickets_customer_idx").on(t.tenantId, t.customerId), index("tickets_assignee_idx").on(t.tenantId, t.assignedMembershipId)]);
...
}, (t) => [index("ticket_comments_ticket_idx").on(t.tenantId, t.ticketId, t.createdAt)]);
...
}, (t) => [uniqueIndex("notification_preferences_event_ux").on(t.customerId, t.eventKey)]);
...
}, (t) => [uniqueIndex("outbound_messages_idempotency_ux").on(t.tenantId, t.idempotencyKey), index("outbound_messages_queue_idx").on(t.tenantId, t.status, t.queuedAt)]);
...
}, (t) => [uniqueIndex("service_feedback_source_ux").on(t.tenantId, t.customerId, t.jobId, t.source)]);
...
}, (t) => [index("consent_records_customer_idx").on(t.tenantId, t.customerId, t.channel, t.category, t.capturedAt)]);
...
}, (t) => [index("automation_rules_status_idx").on(t.tenantId, t.status)]);
...
}, (t) => [uniqueIndex("automation_runs_idempotency_ux").on(t.tenantId, t.idempotencyKey), index("automation_runs_retry_idx").on(t.tenantId, t.status, t.nextRetryAt)]);
...
}, (t) => [uniqueIndex("connector_mappings_provider_ux").on(t.connectorInstallationId, t.resourceType, t.providerResourceId)]);
...
}, (t) => [uniqueIndex("sync_states_installation_type_ux").on(t.connectorInstallationId, t.syncType)]);
...
}, (t) => [uniqueIndex("webhook_events_provider_ux").on(t.connectorKey, t.providerEventId), index("webhook_events_queue_idx").on(t.status, t.receivedAt)]);
...
}, (t) => [index("import_batches_tenant_status_idx").on(t.tenantId, t.status)]);
...
}, (t) => [uniqueIndex("import_rows_batch_number_ux").on(t.importBatchId, t.rowNumber)]);
