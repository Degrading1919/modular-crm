...
function Job({ id, mutate }: { id: string; mutate: (path: string, payload: unknown) => Promise<"saved" | "queued"> }) {
  ...
  const [note, setNote] = useState(""); const [reason, setReason] = useState(""); const [photo, setPhoto] = useState(""); const [photoName, setPhotoName] = useState(""); const [checkProperty, setCheckProperty] = useState(false); const [checkGate, setCheckGate] = useState(false); const [skipOpen, setSkipOpen] = useState(false); const [error, setError] = useState(""); const [busy, setBusy] = useState(false);
  ...
  return <><Link href="/field/today" className="link">← Today’s work</Link>{result.error && <div style={{ marginTop: 15 }}><Notice kind="error" text={result.error}/></div>} {error && <div style={{ marginTop: 15 }}><Notice kind="error" text={error} onClose={() => setError("")}/></div>}{result.loading ? <Loading/> : <><div className="field-greeting" style={{ marginTop: 22 }}><div><div className="eyebrow">{job.serviceName || "Service"}</div><h1>{job.customerName || job.name || "Job"}</h1><p>{job.address || "Address pending"}</p></div><Badge status={job.status}/></div><div className="inline-actions" style={{ margin: "14px 0 21px" }}>{job.address && <a className="btn btn-secondary btn-sm" href={navigationUrl(job.address)} target="_blank" rel="noreferrer"><Icon name="map" size={15}/> Open map</a>}{job.phone && <a className="btn btn-secondary btn-sm" href={`tel:${job.phone}`}><Icon name="person" size={15}/> Call customer</a>}</div><div className="stack"><div className="card card-pad"><h2>Before you begin</h2><div className="detail-list"><dt>Service</dt><dd>{job.serviceName || "Service"}</dd></div><div className="detail-list"><dt>Expected time</dt><dd>{job.durationMinutes ? `${job.durationMinutes} minutes` : "See job instructions"}</dd></div><div className="detail-list"><dt>Access</dt><dd>{job.accessNotes || job.gateAccessMethod || "No special access notes"}</dd></div>{job.gateCode && <div className="detail-list"><dt>Gate code</dt><dd>{job.gateCode}</dd></div>}{job.pets?.length > 0 && <div className="detail-list"><dt>Pets</dt><dd>{job.pets.map((pet: Item) => pet.name).join(", ")}</dd></div>}{job.safetyNotes && <div className="field-warning"><Icon name="warning" size={18}/><span>{job.safetyNotes}</span></div>}{job.notes && <p style={{ margin: "16px 0 0", fontSize: ".84rem" }}>{job.notes}</p>}</div><div className="card card-pad"><h2>Job actions</h2><p className="subtle" style={{ fontSize: ".85rem" }}>Update the customer and office as you work.</p><div className="inline-actions">{["scheduled", "dispatched"].includes(job.status) && <button type="button" className="btn btn-secondary" disabled={busy} onClick={() => action(`/jobs/${id}/transition`, { status: "en_route" })}>On my way</button>}{["scheduled", "dispatched", "en_route", "paused"].includes(job.status) && <button type="button" className="btn btn-primary" disabled={busy} onClick={() => action(`/jobs/${id}/transition`, { status: "in_progress" })}>Start job</button>}{job.status === "in_progress" && <button type="button" className="btn btn-secondary" disabled={busy} onClick={() => action(`/jobs/${id}/transition`, { status: "paused" })}>Pause job</button>}{!["completed", "skipped", "canceled"].includes(job.status) && <button type="button" className="btn btn-danger" disabled={busy} onClick={() => setSkipOpen(true)}>Can’t complete</button>}</div></div>{!["completed", "skipped", "canceled"].includes(job.status) && <><div className="card card-pad"><h2>Notes & proof</h2><div className="field"><label htmlFor="field-note">What should the office know?</label><textarea id="field-note" value={note} onChange={(event) => setNote(event.target.value)} placeholder="Add a service note or mention an issue"/></div><div className="field" style={{ marginTop: 14 }}><label htmlFor="field-photo">Completion photo (optional)</label><input id="field-photo" type="file" accept="image/*" capture="environment" onChange={choosePhoto}/>{photoName && <small>Ready to upload: {photoName}</small>}</div><button type="button" className="btn btn-secondary btn-sm" style={{ marginTop: 16 }} disabled={busy || !note.trim() && !photo} onClick={() => action(`/field/jobs/${id}/note`, { text: note, photoDataUrl: photo || undefined, photoName: photoName || undefined })}>Save note</button></div><div className="card card-pad"><h2>Finish this job</h2><p className="subtle" style={{ fontSize: ".85rem" }}>Confirm these steps before marking service complete.</p><label className="checkbox-row"><input type="checkbox" checked={checkProperty} onChange={(event) => setCheckProperty(event.target.checked)}/>I confirmed the correct property and completed the service.</label><label className="checkbox-row" style={{ marginTop: 12 }}><input type="checkbox" checked={checkGate} onChange={(event) => setCheckGate(event.target.checked)}/>I left gates and access points secure.</label><button type="button" className="btn btn-primary btn-block" style={{ marginTop: 20 }} disabled={busy || !checkProperty || !checkGate || job.status !== "in_progress"} onClick={() => action(`/field/jobs/${id}/complete`, { checklist: { propertyConfirmed: checkProperty, gateSecured: checkGate }, note, photoDataUrl: photo || undefined, photoName: photoName || undefined })}>Complete job <Icon name="check" size={17}/></button>{job.status !== "in_progress" && <small className="subtle" style={{ display: "block", marginTop: 9 }}>Start the job before completing it.</small>}</div></>}</div></>}{skipOpen && <Modal title="Why can’t this job be completed?" onClose={() => setSkipOpen(false)}><div className="field"><label htmlFor="skip-reason">Reason</label><select id="skip-reason" value={reason} onChange={(event) => setReason(event.target.value)}><option value="">Choose a reason</option>{["gate_locked", "unsafe_animal", "customer_requested", "weather", "property_condition", "technician_issue", "address_issue", "other"].map((option) => <option key={option} value={option}>{friendly(option)}</option>)}</select></div><div className="field" style={{ marginTop: 14 }}><label htmlFor="skip-note">More details (optional)</label><textarea id="skip-note" value={note} onChange={(event) => setNote(event.target.value)}/></div>{error && <Notice kind="error" text={error}/>}<div className="modal-footer"><button type="button" className="btn btn-secondary" onClick={() => setSkipOpen(false)}>Back</button><button type="button" className="btn btn-danger" disabled={!reason || busy} onClick={() => action(`/field/jobs/${id}/skip`, { reason, note })}>Record skip</button></div></Modal>}</>;
}
