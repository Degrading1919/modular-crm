...
import CustomerChangeRequests from "./CustomerChangeRequests";

type Entity = Record<string, any> & { id: string };
type Field = { key: string; label: string; type?: "text" | "email" | "tel" | "date" | "number" | "money" | "textarea" | "select"; options?: string[]; optionLabels?: Record<string, string>; required?: boolean };
  ...
  return <div className="app-layout">
    ...
    <main className="app-main"><header className="topbar"><div className="topbar-left"><button type="button" className="icon-button mobile-menu" aria-label="Open menu" onClick={() => setMenuOpen(true)}><Icon name="menu"/></button><span className="location-chip"><Icon name="building" size={16}/>{tenantName}</span><span className="topbar-date">{new Intl.DateTimeFormat("en-US", { weekday: "long", month: "long", day: "numeric" }).format(new Date())}</span></div><div className="topbar-right"><Link href="/app/tickets" className="icon-button" aria-label="Requests"><Icon name="bell"/></Link><div className="avatar" title={displayedName}>{displayedName.slice(0, 1)}</div></div></header><div className="content">
      {waitingForCapabilities ? <Loading label="Checking your workspace tools…"/> : routeBlocked ? <CapabilityUnavailable features={features} featureKeys={gatedFeatures}/> : routePermissionBlocked ? <Empty title="You don’t have access to franchise settings" description="Ask a business owner to manage franchise units and royalty statements." action={<Link className="btn btn-primary" href="/app/dashboard">Back to overview</Link>}/> : <>
        ...
        {resources[current] && current !== "inventory" && current !== "automations" && <ResourcePage resourceKey={current} resource={resources[current]} id={section[1]} features={features} permissions={identity.data.user?.permissions} />}
...
function ResourcePage({ resourceKey, resource, id, features, permissions = [] }: { resourceKey: string; resource: Resource; id?: string; features: Record<string, CapabilityFeature>; permissions?: string[] }) {
  ...
  return <><Header eyebrow={id ? resource.title : "Your business"} title={id ? (current.name || current.subject || current.number || current.customerName || resource.singular) : resource.title} subtitle={id ? `View ${resource.singular.toLowerCase()} details and next steps.` : resource.subtitle}>{id ? <Link href={`/app/${resourceKey}`} className="btn btn-secondary"><Icon name="arrow" size={16} className="rotate-left"/> Back to {resource.title.toLowerCase()}</Link> : <button type="button" className="btn btn-primary" onClick={() => setCreating(true)}><Icon name="plus" size={17}/> New {resource.singular.toLowerCase()}</button>}</Header>
    ...
    {revisingEstimate && resourceKey === "estimates" && <ReviseEstimateForm estimate={current} onClose={() => setRevisingEstimate(false)} onRevised={() => { setRevisingEstimate(false); setSecureEstimateUrl(""); detail.reload(); list.reload(); setNotice("Estimate revision saved. It’s ready to send."); }}/>}
  </>;
}
...
function CreateRecord({ resource, onClose, onCreated }: { resource: Resource; onClose: () => void; onCreated: (item: Entity) => void }) {
  ...
  const industry = useResource<{ item: Entity }>(resource.fields.some((field) => field.key === "frequency") ? "/industry-configuration" : null, { item: { id: "industry-configuration", pack: null } });
  const hasPack = Boolean(industry.data.item?.pack);
  const recurrencePresets: Array<{ key: string; label: string; rrule?: string }> = industry.data.item?.pack?.recurrencePresets ?? industry.data.item?.pack?.pack?.recurrencePresets ?? [];
  const fields = resource.fields.map((field) => field.key === "frequency" && hasPack
    ? { ...field, options: recurrencePresets.filter((preset) => preset.rrule).map((preset) => preset.key), optionLabels: Object.fromEntries(recurrencePresets.filter((preset) => preset.rrule).map((preset) => [preset.key, preset.label])) }
    : field);
  ...
  return <Modal title={`New ${resource.singular.toLowerCase()}`} onClose={onClose}><form onSubmit={submit}><div className="form-grid">{fields.map((field) => <div className={`field ${field.type === "textarea" ? "full" : ""}`} key={field.key}><label htmlFor={`new-${field.key}`}>{field.key === "customerId" ? "Customer" : field.key === "serviceId" ? "Service" : field.label}</label>{field.key === "customerId" || field.key === "serviceId" ? <select id={`new-${field.key}`} value={values[field.key] || ""} required={field.required} onChange={(event) => setValues({ ...values, [field.key]: event.target.value })}><option value="">Choose {field.key === "customerId" ? "customer" : "service"}</option>{unwrapItems(field.key === "customerId" ? customers.data : services.data).map((item) => <option key={item.id} value={item.id}>{item.name || item.title}</option>)}</select> : field.type === "textarea" ? <textarea id={`new-${field.key}`} required={field.required} value={values[field.key] || ""} onChange={(event) => setValues({ ...values, [field.key]: event.target.value })}/> : field.type === "select" ? <><select id={`new-${field.key}`} value={values[field.key] || ""} required={field.required} disabled={field.key === "frequency" && hasPack && !field.options?.length} onChange={(event) => setValues({ ...values, [field.key]: event.target.value })}><option value="">Choose one</option>{field.options?.map((option) => <option key={option} value={option}>{field.optionLabels?.[option] || friendly(option)}</option>)}</select>{field.key === "frequency" && hasPack && !field.options?.length && <small>This business has no supported recurring schedule preset yet. Contact the office to arrange one.</small>}</> : <input id={`new-${field.key}`} type={field.type === "money" || field.type === "number" ? "number" : field.type || "text"} min={field.type === "money" || field.type === "number" ? 0 : undefined} step={field.type === "money" ? ".01" : undefined} required={field.required} value={values[field.key] || ""} onChange={(event) => setValues({ ...values, [field.key]: event.target.value })}/>}</div>)}</div>{(customers.error || services.error || industry.error) && <div style={{ marginTop: 15 }}><Notice kind="error" text={customers.error || services.error || industry.error}/></div>}{error && <div style={{ marginTop: 15 }}><Notice kind="error" text={error}/></div>}<div className="modal-footer"><button type="button" className="btn btn-secondary" onClick={onClose}>Cancel</button><button type="submit" className="btn btn-primary" disabled={saving}>{saving ? "Saving…" : `Create ${resource.singular.toLowerCase()}`}</button></div></form></Modal>;
}

function ReviseEstimateForm({ estimate, onClose, onRevised }: { estimate: Entity; onClose: () => void; onRevised: () => void }) {
  const revision = estimate.currentRevisionData || {};
  const [title, setTitle] = useState(String(revision.title || ""));
  const [total, setTotal] = useState((Number(revision.totalCents ?? estimate.totalCents ?? 0) / 100).toFixed(2));
  ...
  async function submit(event: React.FormEvent) {
    ...
    try {
      await api(`/estimates/${estimate.id}`, patch({ title, totalCents: Math.round(Number(total) * 100), notes }));
      onRevised();
    } catch (issue) {
  ...
  return <Modal title="Revise estimate" onClose={onClose}><form onSubmit={submit}><p className="subtle">This creates a new version and keeps previous versions for your records.</p><div className="form-grid"><div className="field full"><label htmlFor="estimate-revision-title">What is included?</label><input id="estimate-revision-title" required maxLength={500} value={title} onChange={(event) => setTitle(event.target.value)}/></div><div className="field"><label htmlFor="estimate-revision-total">Total price</label><input id="estimate-revision-total" type="number" min="0" step="0.01" required value={total} onChange={(event) => setTotal(event.target.value)}/></div><div className="field full"><label htmlFor="estimate-revision-notes">Description</label><textarea id="estimate-revision-notes" maxLength={10000} value={notes} onChange={(event) => setNotes(event.target.value)}/></div></div>{error && <div style={{ marginTop: 15 }}><Notice kind="error" text={error}/></div>}<div className="modal-footer"><button type="button" className="btn btn-secondary" onClick={onClose}>Cancel</button><button type="submit" className="btn btn-primary" disabled={saving}>{saving ? "Saving…" : "Save new revision"}</button></div></form></Modal>;
}
...
function Connections({ role, permissions }: { role: string; permissions?: string[] }) {
  const result = useResource<{ items: Entity[]; industryPack?: { key: string; displayName: string; recommendedConnectorCapabilities: string[] } }>("/connections", { items: [] });
  const { loading: connectionsLoading, reload: reloadConnections } = result;
  ...
  const industryPack = result.data?.industryPack;
  ...
  useEffect(() => {
    if (connectionsLoading || typeof window === "undefined") return;
    ...
    reloadConnections();
  }, [connectionsLoading, items, reloadConnections]);
...
function Settings() {
  ...
  return <><Header eyebrow="Your workspace" title="Settings" subtitle="Keep the business details your team and customers rely on current."/>{result.error && <Notice kind="error" text={result.error}/>} {error && <Notice kind="error" text={error}/>} {notice && <Notice kind="success" text={notice}/>} {result.loading ? <Loading/> : <><div className="detail-grid"><form className="card card-pad" onSubmit={submit}><h2>Business profile</h2><div className="form-grid">{[{ key: "businessName", label: "Business name", type: "text" }, { key: "phone", label: "Phone", type: "tel" }, { key: "email", label: "Email", type: "email" }, { key: "timezone", label: "Time zone", type: "text" }, { key: "address", label: "Business address", type: "text" }].map((field) => <div className="field" key={field.key}><label htmlFor={`settings-${field.key}`}>{field.label}</label><input id={`settings-${field.key}`} type={field.type} value={values[field.key] || ""} onChange={(event) => setValues({ ...values, [field.key]: event.target.value })}/></div>)}</div><div style={{ marginTop: 24 }}><button className="btn btn-primary" type="submit" disabled={saving}>{saving ? "Saving…" : "Save business profile"}</button></div></form><div className="stack"><div className="card card-pad"><h3>Bring your data</h3><p className="subtle" style={{ fontSize: ".85rem" }}>Upload a spreadsheet of customers. We’ll help match columns before importing.</p><Link href="/app/import" className="link">Import customers</Link></div><div className="card card-pad"><h3>Team and access</h3><p className="subtle" style={{ fontSize: ".85rem" }}>Invite staff and choose an owner, office, or technician role.</p><Link href="/app/staff" className="link">Manage staff</Link></div></div></div><IndustryPackSettings/></>}</>;
}

function IndustryPackSettings() {
  const result = useResource<{ item: Entity }>("/industry-configuration", { item: { id: "industry-configuration", pack: null } });
  const item = unwrapItem(result.data);
  const pack = item.pack?.pack;
  const resolved = item.pack;
  const canUpdate = item.permissions?.canUpdate === true;
  const [customization, setCustomization] = useState<Record<string, any>>({});
  const [error, setError] = useState(""); const [notice, setNotice] = useState(""); const [saving, setSaving] = useState(false);
  useEffect(() => { if (!result.loading && !result.error) setCustomization(item.customization || {}); }, [result.loading, result.error, item.customization]);
  function changeField(group: "locationFields" | "assetFields", key: string, property: "enabled" | "required", value: boolean, assetKey?: string) {
    setCustomization((current) => {
      const next = { ...current };
      if (group === "locationFields") next.locationFields = { ...(current.locationFields || {}), [key]: { ...(current.locationFields?.[key] || {}), [property]: value } };
      else next.assetFields = { ...(current.assetFields || {}), [assetKey!]: { ...(current.assetFields?.[assetKey!] || {}), [key]: { ...(current.assetFields?.[assetKey!]?.[key] || {}), [property]: value } } };
      return next;
    });
  }
  function changeChecklist(key: string, property: "enabled" | "required", value: boolean) {
    setCustomization((current) => ({ ...current, checklist: { ...(current.checklist || {}), [key]: { ...(current.checklist?.[key] || {}), [property]: value } } }));
  }
  function changeRecurrence(key: string, enabled: boolean) {
    setCustomization((current) => {
      const disabled = new Set<string>(current.disabledRecurrencePresetKeys || []);
      if (enabled) disabled.delete(key); else disabled.add(key);
      return { ...current, disabledRecurrencePresetKeys: [...disabled] };
    });
  }
  async function save(event: React.FormEvent) {
    event.preventDefault(); setSaving(true); setError(""); setNotice("");
    try { await api("/industry-configuration", patch({ customization })); setNotice("Business workflow settings saved. Existing jobs keep their recorded checklist."); result.reload(); }
    catch (issue) { setError((issue as Error).message); }
    finally { setSaving(false); }
  }
  function fieldControls(field: Entity, group: "locationFields" | "assetFields", assetKey?: string) {
    const setting = group === "locationFields" ? customization.locationFields?.[field.key] || {} : customization.assetFields?.[assetKey!]?.[field.key] || {};
    const enabled = setting.enabled !== false;
    const required = setting.required ?? field.required ?? false;
    return <div className="action-item" key={`${assetKey || "location"}:${field.key}`} style={{ alignItems: "center" }}><div style={{ flex: 1 }}><strong>{field.label}</strong><p className="subtle" style={{ margin: "3px 0 0", fontSize: ".78rem" }}>{field.sensitive ? "Protected information" : field.type.replaceAll("_", " ")}</p></div><label className="checkbox-row" style={{ fontSize: ".8rem" }}><input type="checkbox" checked={enabled} disabled={!canUpdate} onChange={(event) => changeField(group, field.key, "enabled", event.target.checked, assetKey)}/>Use this field</label><label className="checkbox-row" style={{ fontSize: ".8rem" }}><input type="checkbox" checked={required} disabled={!canUpdate || !enabled} onChange={(event) => changeField(group, field.key, "required", event.target.checked, assetKey)}/>Required</label></div>;
  }
  return <section className="card card-pad" aria-label="Business-specific workflow settings" style={{ marginTop: 18 }}><div className="card-heading"><div><div className="eyebrow">Built around your business</div><h2>{pack?.displayName || "Industry workflow"}</h2></div>{item.maturity && <span className="badge badge-neutral">{item.maturity === "validated" ? "Representative workflow validated" : "Runtime configured"}</span>}</div>{result.error && <Notice kind="error" text={result.error}/>} {error && <Notice kind="error" text={error}/>} {notice && <Notice kind="success" text={notice}/>} {result.loading ? <Loading/> : !pack ? <p className="subtle">Choose an industry during setup to customize the information your team collects and the steps required to finish work.</p> : <form onSubmit={save}><p className="subtle" style={{ fontSize: ".86rem" }}>Choose which business details matter to your team. New intake and new job checklists use these settings; completed work keeps its original record.</p>{resolved.locationFields?.length > 0 && <><div className="section-title"><h3>Service location details</h3></div>{resolved.locationFields.map((field: Entity) => fieldControls(field, "locationFields"))}</>}{resolved.assets?.map((asset: Entity) => <div key={asset.key}><div className="section-title"><h3>{asset.pluralLabel}</h3></div>{asset.fields.map((field: Entity) => fieldControls(field, "assetFields", asset.key))}</div>)}{resolved.jobChecklist?.length > 0 && <><div className="section-title"><h3>Work completion steps</h3></div>{resolved.jobChecklist.map((step: Entity) => { const setting = customization.checklist?.[step.key] || {}; return <div className="action-item" key={step.key} style={{ alignItems: "center" }}><div style={{ flex: 1 }}><strong>{step.label}</strong></div><label className="checkbox-row" style={{ fontSize: ".8rem" }}><input type="checkbox" checked={setting.enabled !== false} disabled={!canUpdate} onChange={(event) => changeChecklist(step.key, "enabled", event.target.checked)}/>Use this step</label><label className="checkbox-row" style={{ fontSize: ".8rem" }}><input type="checkbox" checked={setting.required ?? step.required} disabled={!canUpdate || setting.enabled === false} onChange={(event) => changeChecklist(step.key, "required", event.target.checked)}/>Required</label></div>; })}</>}{resolved.recurrencePresets?.length > 0 && <><div className="section-title"><h3>Repeat schedules</h3></div>{resolved.recurrencePresets.map((preset: Entity) => <label className="checkbox-row" key={preset.key} style={{ margin: "9px 0" }}><input type="checkbox" checked={!(customization.disabledRecurrencePresetKeys || []).includes(preset.key)} disabled={!canUpdate} onChange={(event) => changeRecurrence(preset.key, event.target.checked)}/><span><strong>{preset.label}</strong>{preset.custom && <small className="subtle" style={{ display: "block" }}>Set this schedule with your team</small>}</span></label>)}</>}{canUpdate && <div style={{ marginTop: 20 }}><button type="submit" className="btn btn-primary" disabled={saving}>{saving ? "Saving…" : "Save workflow settings"}</button></div>}</form>}</section>;
}
