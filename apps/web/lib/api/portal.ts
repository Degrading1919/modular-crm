...
import { z } from "zod";
import { customerAssets, customerChangeRequests, customerContacts, customerPreferences, customers, serviceFeedback, ticketStatusDefinitions, ticketTypeDefinitions, tickets } from "@modular-crm/db";
...
async function profile(actor: SessionActor) {
  ...
  const [contacts, locations, pets, preferences] = await Promise.all([
    ...
    rows(sql`select id,address_line1 as address,city,region,postal_code from service_locations where tenant_id=${actor.tenantId} and customer_id=${id} and id=any(${uuidArray(actor.locationIds)}) order by created_at`),
    rows(sql`select id,name,custom_fields from customer_assets where tenant_id=${actor.tenantId} and customer_id=${id} and archived_at is null and customer_visible=true and (service_location_id is null or service_location_id=any(${uuidArray(actor.locationIds)})) order by created_at`),
    db.select().from(customerPreferences).where(and(eq(customerPreferences.tenantId, actor.tenantId), eq(customerPreferences.customerId, id))),
  ]);
  ...
  return normalized({ id, name: contact ? [contact.first_name, contact.last_name].filter(Boolean).join(" ") : customer.displayName,
    ...
    notificationPreferences: preferences.find((item) => item.preferenceKey === "notifications")?.value ?? { email: true, sms: false },
...
async function updateProfile(request: Request, actor: SessionActor): Promise<Response> {
  ...
  await db.transaction(async (tx) => {
    ...
    if (body.notificationPreferences) await tx.insert(customerPreferences).values({ tenantId: actor.tenantId, customerId: id, preferenceKey: "notifications", value: body.notificationPreferences }).onConflictDoUpdate({ target: [customerPreferences.tenantId, customerPreferences.customerId, customerPreferences.preferenceKey], set: { value: body.notificationPreferences, updatedAt: new Date() } });
    if (body.pets) for (const pet of body.pets) {
      ...
      if (!existing || (existing.serviceLocationId && !actor.locationIds.has(existing.serviceLocationId))) throw new DomainError("NOT_FOUND", "Pet not found.", 404);
...
async function portalList(actor: SessionActor, resource: string): Promise<Response> {
  ...
  switch (resource) {
    case "services": items = await rows(sql`select sp.*,s.name as service_name,rr.frequency_type as frequency,sl.address_line1 as address,
      ...
      where sp.tenant_id=${actor.tenantId} and sp.customer_id=any(${uuidArray(customerIds)}) and sp.service_location_id=any(${uuidArray(actor.locationIds)}) order by sp.created_at desc`); break;
    case "visits": items = await rows(sql`select j.id,j.service_plan_id,j.service_id,j.status,j.scheduled_date,j.actual_completed_at,j.customer_summary,
      ...
      where j.tenant_id=${actor.tenantId} and j.customer_id=any(${uuidArray(customerIds)})
        and j.service_location_id=any(${uuidArray(actor.locationIds)}) and j.status='completed'
    ...
    case "estimates": items = await rows(sql`select e.*,('EST-' || left(e.id::text,8)) as number,e.total_minor as total_cents,er.snapshot->>'title' as title,er.notes
      ...
      where e.tenant_id=${actor.tenantId} and e.customer_id=any(${uuidArray(customerIds)}) and e.status<>'draft'
      and exists (select 1 from service_locations sl where sl.tenant_id=e.tenant_id and sl.customer_id=e.customer_id
        and sl.organization_location_id=e.organization_location_id and sl.id=any(${uuidArray(actor.locationIds)}))
    ...
    case "invoices": items = await rows(sql`select i.*,i.invoice_number as number,i.total_minor as total_cents,i.balance_minor as balance_cents,i.due_at as due_date
      ...
      and exists (select 1 from service_locations sl where sl.tenant_id=i.tenant_id and sl.customer_id=i.customer_id
        and sl.organization_location_id=i.organization_location_id and sl.id=any(${uuidArray(actor.locationIds)}))
    ...
    case "tickets": items = await rows(sql`select t.id,t.title as subject,t.description,ts.key as status,t.created_at from tickets t
      ...
      where t.tenant_id=${actor.tenantId} and t.customer_id=any(${uuidArray(customerIds)}) and t.customer_visible=true order by t.created_at desc`); break;
    case "requests": items = await rows(sql`select id,request_type as type,customer_message as details,status,created_at from customer_change_requests
      where tenant_id=${actor.tenantId} and customer_id=any(${uuidArray(customerIds)}) order by created_at desc`); break;
    case "files": items = await rows(sql`select f.id,f.original_name as name,f.mime_type,f.created_at,fl.purpose as description
      ...
      where f.tenant_id=${actor.tenantId} and j.customer_id=any(${uuidArray(customerIds)}) and j.service_location_id=any(${uuidArray(actor.locationIds)}) and f.visibility='customer' order by f.created_at desc`); break;
...
async function createRequest(request: Request, actor: SessionActor): Promise<Response> {
  ...
  if (body.servicePlanId) {
    ...
    if (!plan.length || !actor.locationIds.has(String(plan[0]!.service_location_id))) throw new DomainError("NOT_FOUND", "Service plan not found.", 404);
    serviceLocationId = String(plan[0]!.service_location_id);
  }
  const [created] = await getDb().insert(customerChangeRequests).values({ tenantId: actor.tenantId, customerId: id, serviceLocationId, requestType: body.type, status: "pending", requestedChanges: { servicePlanId: body.servicePlanId, effectiveDate: body.effectiveDate }, customerMessage: body.details, submittedByUserId: actor.userId }).returning();
  await recordEvent(actor, { type: "customer.requested_change", entityType: "customer_change_request", entityId: created!.id, payload: { customerId: id, type: body.type }, auditAction: "customer.request_change" });
